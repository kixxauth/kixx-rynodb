...
class DynamoDB {
	...
	getEntity(key, options = {}) {
		...
		return this.client.requestWithBackoff('GetItem', params, options).then((res) => {
			return {entity: DynamoDB.deserializeObject(res.Item)};
		});
	}
	...
	// options.tablePrefix
	// options.client - DynamoDbCLient instance.
	// options.emitter - EventEmitter instance
	static create(options = {}) {
		if (!options.emitter || typeof options.emitter.emit !== 'function') {
			throw new Error(`expects options.emitter to be an EventEmitter`);
		}

		if (!options.client || typeof options.client.requestWithBackoff !== 'function') {
			throw new Error(`expects options.client to be a DynamoDbCLient`);
		}
