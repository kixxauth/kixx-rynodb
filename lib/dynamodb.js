...
class DynamoDB {
	...
	setupSchema(options = {}) {
		const {client, emitter, rootEntityTable, relationshipEntriesTable, indexEntriesTable} = this;
		...
		const describeRelationshipEntriesTable = () => {
			return client.request('DescribeTable', {TableName: relationshipEntriesTable}, options);
		};
