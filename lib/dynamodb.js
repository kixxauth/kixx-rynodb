...
const {clone, compact, isNonEmptyString, partition} = require('kixx/library');
...
class DynamoDB {
	...
	batchWrite(TableName, options, requests) {
		...
		const backoff = DynamoDB.computeBackoffMs(this.getBackoffMultiplier(options));
		...
		function tryOperation(retryCount) {
			return request(params).catch((err) => {
				if (err.code === ProvisionedThroughputExceededException) {
					...
					const err = new Error(
						`encountered ProvisionedThroughputExceededException on ${tableMessage} during ${target}`
					);
					err.code = THROUGHPUT_EXCEEDED;
					emitter.emit('warning', err);
