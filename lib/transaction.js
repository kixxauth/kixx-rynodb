...
function createEntity(entity, options) {
	return this.dynamodb.createEntity(entity, options).then(function (res) {
		return Entity.fromDatabaseRecord(res.entity);
	});
}

function updateEntity(entity, options) {
	return this.dynamodb.updateEntity(entity, options).then(function (res) {
		return Entity.fromDatabaseRecord(res.entity);
	});
}
...
function updateIndexEntries(toRemove, toCreate, options) {
	...
	const keysToCreate = toCreate.map((entry) => entry.getFullKey());

	const entriesToRemove = toRemove.filter((entry) => {
		return !keysToCreate.includes(entry.getFullKey());
	});

	return this.dynamodb.updateIndexEntries(entriesToRemove, toCreate, options);
}
...
class Transaction {
	constructor(options) {
		Object.defineProperties(this, {
			...
			_createEntity: {
				value: createEntity.bind(this)
			},
			_updateEntity: {
				value: updateEntity.bind(this)
			},
			_getIndexEntries: {
	...
	createItem(object, options = {}) {
		...
		const {_composeIndexEntries, _createEntity, _createIndexEntries} = this;

		const newEntity = Entity.fromPublicObject(object).setUpdate();
		const indexEntries = _composeIndexEntries(object);
	...
	updateItem(object, options = {}) {
		...
		assert.isNonEmptyString(scope, 'updateItem() object.scope');
		assert.isNonEmptyString(type, 'updateItem() object.type');
		assert.isNonEmptyString(id, 'updateItem() object.id');
		...
		const entity = Entity.fromPublicObject(object).setUpdate();
