	...
	return function createTransaction() {
		...
		function get(args) {
			...
			return promise.then((res) => {
				...
				if (data.relationships && include && include.length > 0) {
					...
					// return statement.
					if (keys.length > 0) {
						// Try to fetch objects from the cache first.
		...
		function set(args) {
			...
			return setObject(api, options, scope, record).then((res) => {
				...
				original.then((originalObject) => {
					if (!originalObject) return false;
					updateForeignKeys(scope, originalObject, data).catch((err) => {
		...
		function batchSet(args) {
			...
			// after we set it.
			let originals;
			const cachedResults = uniqueByKey(compact(transactionCache.batchGet(scope, records).data));
			...
			const toFetch = differenceByKey(objects, cachedResults);
			if (toFetch.length > 0) {
				originals = batchGetObjects(api, options, scope, toFetch).then((res) => {
					return cachedResults.concat(compact(res.data));
				});
			} else {
				originals = Promise.resolve(cachedResults);
			}
			...
			return batchSetObjects(api, options, scope, records).then((res) => {
				...
				// Here we update foreign keys in the background while allowing the
				// response to be returned.
				originals.then((original) => {
					uniqueByKey(data).reduce((promise, obj) => {
						const {type, id} = obj;
						return promise.then(() => {
		...
		function removeForeignKeyRelationships(scope, obj) {
			...
			console.log(`removeForeignKeyRelationships`);
			console.log(obj);
