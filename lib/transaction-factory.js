	...
	return function createTransaction() {
		...
		function batchSet(args) {
			...
			return batchSetObjects(api, options, scope, records).then((res) => {
				...
				const toFetch = differenceByKey(data, compact(cachedResults.data));

				let objectsToUpdate;
				if (toFetch.length > 0) {
					objectsToUpdate = batchGetObjects(api, options, scope, toFetch).then((res) => {
						return uniqueByKey(cachedResults.data).concat(uniqueByKey(res.data));
					});
				} else {
					objectsToUpdate = Promise.resolve(uniqueByKey(cachedResults.data));
				}
				...
				// Here we update foreign keys in the background while allowing the
				// response to be returned.
				objectsToUpdate.then((existing) => {
					uniqueByKey(data).reduce((promise, obj) => {
						return promise.then(() => {
							const existingObject = find(hasKey({type: obj.type, id: obj.id}), existing);
							if (existingObject) {
								return updateForeignKeys(scope, existingObject, obj);
							}
							return false;
						});
