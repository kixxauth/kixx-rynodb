	...
	return function createTransaction() {
		...
		function batchSet(args) {
			...
			return batchSetObjects(api, options, scope, records).then((res) => {
				...
				// Find the set (no duplicates) of keys which are in retrieved records but not
				// in the cached results. These are keys which must be fetched.
				const toFetch = differenceByKey(data, cachedResults.data);

				let objectsToUpdate;
				if (toFetch.length > 0) {
					objectsToUpdate = batchGetObjects(api, options, scope, toFetch).then((res) => {
						return cachedResults.data.concat(res.data);
					});
				} else {
					objectsToUpdate = Promise.resolve(cachedResults.data);
				}
