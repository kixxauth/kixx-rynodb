...
module.exports = function transactionFactory(options) {
	...
	if (!isFunction(events.broadcast)) {
		throw new ProgrammerError(
			`transactionFactory() options must include a valid events`
		);
	}
	...
	if (!isNonEmptyString(options.dynamodbTablePrefix) || /^[a-z_]+$/.test(options.dynamodbTablePrefix)) {
		throw new ProgrammerError(
	...
	return function createTransaction() {
		...
		function set(args) {
			...
			const record = Record.create(object);
		...
		function batchSet(args) {
			...
			const records = objects.map(Record.create);
		...
		function batchRemove(args) {
			...
			return batchGetObjects(api, options, scope, missedKeys).then((res) => {
				...
				const data = keys.map((key, i) => {
					if (cachedResults.data[i]) {
						return Record.create(cachedResults.data[i]);
					}
					return find(hasKey(key), res.data);
				});
		...
		function updateForeignKeys(scope, oldObject, newObject) {
			oldObject = Record.create(oldObject);
			newObject = Record.create(newObject);
...
function createTransactionCache() {
	...
	function set(scope, obj) {
		const {type, id} = obj;
		const k = `${scope}:${type}:${id}`;
		cache[k] = Record.create(clone(obj));
