...
// Main export
//
// - `options.eventBus` *required* Event Bus Object
...
module.exports = function transactionFactory(options) {
	...
	if (!eventBus || !isFunction(eventBus.broadcast)) {
		throw new ProgrammerError(
			`transactionFactory() options must include a valid eventBus channel (Object)`
		);
	}
	...
	return function createTransaction() {
		...
		// - `args.scope` *required*
		// - `args.keys[].type` *required*
		// - `args.keys[].id` *required*
		// - `args.options.useCache` (default=true)
		// - `args.options.resetCache` (default=true)
		...
		// - `args.scope` *required*
		// - `args.objects[].type` *required*
		// - `args.objects[].id` *required*
		// - `args.isolated` (default=false)
		// - `args.options.useCache` (default=false)
		// - `args.options.resetCache` (default=true)
		//
		...
		function scan(args) {
			...
			// We don't intend to use any cache at all during a scan operation. We
			// read directly from the source database index.
			return api.dynamodbScanQuery(options, scope, type).then((res) => {
				const {data, cursor, meta} = res;
				returnMeta = append(meta, returnMeta);

				// Set the transaction cache for future non-scan method calls on this
				// transaction instance.
				transactionCache.batchSet(scope, data);

				return Response.create({
					// Send back the ResourceObject instances expected by the caller.
					data: data.map(ResourceObject.create),
					cursor,
					meta: returnMeta
				});
		...
		// Return the public transaction API.
		return {
			get,
			batchGet,
			set,
