...
class DynamoDbClient {
	...
	_batchWrite(TableName, requests, options) {
		...
		// TODO: Property bind the request method with arguments.
		const request = this._requestWithBackoff.bind(this);
		...
		function tryOperation(params, retryCount) {
			return request(params).then((res) => {
				...
				if (UnprocessedItems && UnprocessedItems[TableName]) {
					const err = new Error(
						`DynamoDbClient encountered UnprocessedItems while running BatchWriteItem on table ${TableName}`
					);
					...
					if (operationTimeout && Date.now() + delay - start >= operationTimeout) {
						const err = new Error(
							`DynamoDbClient operation timeout error during BatchWriteItem due to throttling on table ${TableName}`
						);
		...
		function tryOperation(retryCount) {
			return request(params).catch((err) => {
				if (err.code === ProvisionedThroughputExceededException) {
					...
					const err = new Error(
						`DynamoDbClient encountered ProvisionedThroughputExceededException on ${tableMessage} during ${target}`
					);
					err.code = THROUGHPUT_EXCEEDED;
					emitter.emit('warning', err);
					...
					if (operationTimeout && Date.now() + delay - start >= operationTimeout) {
						const err = new Error(
							`DynamoDbClient operation timeout error during ${target} due to throttling on ${tableMessage} `
						);
						err.code = OPERATION_TIMEOUT;
						return Promise.reject(err);
					}
				...
				return Promise.reject(new StackedError(
					`Unexpected error during DynamoDbClient#requestWithBackoff()`,
					err
				));
			});
		}
