...
exports.create = (options) => {
	options = options || {};
	const thisEmitter = options.emitter || new EventEmitter();

	const thisDynamoDbEndpointString = isNonEmptyString(options.dynamodbEndpoint)
		? options.dynamodbEndpoint
		: `https://dynamodb.${thisAwsRegion}.amazonaws.com`;

	const thisSocketInactivityTimeout = Number.isInteger(options.requestTimeout) ? options.requestTimeout : DEFAULTRE;
	...
	function mergeOptions(opts) {
		return Object.freeze({
			emitter: opts.emitter ? opts.emitter : thisEmitter,
			requestTimeout: Number.isInteger(opts.requestTimeout) ? opts.requestTimeout : thisRequestTimeout,
			operationTimeout: Number.isInteger(opts.operationTimeout) ? opts.operationTimeout : thisOperationTimeout,
			backoffMultiplier: Number.isInteger(opts.backoffMultiplier) ? opts.backoffMultiplier : this.backoffMultiplier,
		});
	}
	...
	return {
		on(event, handler) {
			thisEmitter.on(event, handler);
		},

		off(event, handler) {
			thisEmitter.off(event, handler);
		},

		once(event, handler) {
			thisEmitter.once(event, handler);
		},
