...
const {assert, complement, isEmpty, isObject} = require(`kixx/library`);
...
module.exports = function createEntity(dynamodb, redis, options, emitRollback, subject) {
	...
	const {scope, type, id, attributes, meta} = subject;
	...
	const Item = DynamoDB.newRootRecord({
		scope,
		type,
		id,
		created,
	...
	promises.push(
		dynamodb.createItem(options, {
			...
			emitRollback(function createItemRollback() {
				const Key = DynamoDB.keyFromRootRecord(Item);
