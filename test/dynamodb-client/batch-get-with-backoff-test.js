...
module.exports = function (t) {
	t.describe('nominal case', (t) => {
		...
		const response = Object.freeze({UnprocessedKeys: {}});
		...
		t.before(function (done) {
			...
			return client.batchGetWithBackoff(target, params, options)
				.then(function (res) {
					result = res;
					return done();
				})
	...
	t.describe('with ProvisionedThroughputExceededException and UnprocessedKeys', (t) => {
		let emitter;
		let client;
		...
		t.before(function (done) {
			...
			sinon.stub(client, '_request')
				...
				.onCall(1).callsFake(() => {
					...
					return Promise.resolve(Object.freeze({UnprocessedKeys: {SomeTable: 1}}));
				})
				.onCall(2).callsFake(() => {
		...
		t.it('emits 2 warnings', () => {
			...
			assert.isEqual('DynamoDB UnprocessedKeys during SomeTarget on table SomeTable', args2[0].message);
		});
	...
	t.describe('with UnprocessedKeys => operation timeout', (t) => {
		let emitter;
		let client;
		...
		t.before(function (done) {
			...
			sinon.stub(client, '_request')
				.onCall(0).returns(Promise.resolve(Object.freeze({UnprocessedKeys: {SomeTable: 1}})))
	...
	t.describe('with ProvisionedThroughputExceededException and UnprocessedKeys => operation timeout = 4s', (t) => {
		let emitter;
		let client;
		...
		t.before(function (done) {
			...
			sinon.stub(client, '_request')
				...
				.onCall(1).returns(Promise.resolve(Object.freeze({UnprocessedKeys: {SomeTable: 1}})))
				.onCall(2).returns(Promise.resolve(Object.freeze({UnprocessedKeys: {SomeTable: 1}})))
	...
	t.describe('with ProvisionedThroughputExceededException and UnprocessedKeys => operation timeout fail', (t) => {
		let emitter;
		let client;
